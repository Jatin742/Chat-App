    ...
    return (
        <div className='mt-5'>
            {contacts.map((contact) => {
                const strategy = ContactStrategyFactory.getStrategy(contact.type);
                const details = strategy.getDetails(contact);
                return (<div key={contact._id} className={`pl-10 py-2 transition-all duration-300 cursor-pointer ${selectedChatData?._id === contact._id ? 'bg-[#8417ff] hover:bg-[#8417ff]' : 'hover:bg-[#f1f1f111] '}`} onClick={() => handleContactClick(contact)}>
